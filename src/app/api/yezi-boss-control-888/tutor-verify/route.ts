...
export async function GET() {
  ...
  const { data: rows, error } = await supabase
    .from("tutor_posts")
    .select("id, real_name, university, identity, gender, teach_mode, regions, grades, subjects, min_salary, max_salary, note, teaching_style, auth_files, status, serial_number, created_at")
    .eq("status", "pending")
