...
// 管理员自定义勋章文案（存入 tutor_posts.badge_text）
const BADGE_OPTIONS = [
  "实名认证",
  "优秀教员",
  "金牌教员",
...
export default function AdminVerifyPage() {
  ...
  const [editingTutorBadge, setEditingTutorBadge] = useState<string>("");
  ...
  const [publishBadge, setPublishBadge] = useState<string>("");
  ...
  async function handlePublishSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (publishType === "tutor") {
        ...
        body.badge_text = publishBadge || (publishType === "tutor" ? "实名认证" : null);
  ...
  function openTutorEdit(row: PendingRow) {
    ...
    setEditingTutorBadge((row.badge_text as string) || (row.status === "verified" ? "实名认证" : ""));
        ...
        <form onSubmit={handlePublishSubmit} className="mt-4 space-y-4">
          ...
          {publishType === "tutor" && (
            <>
              ...
              <div>
                <label className="block text-xs text-muted-foreground">学历</label>
                <div className="mt-1 flex flex-wrap gap-2">
                  {IDENTITY_OPTIONS.map(({ value, label }) => (
                    <button
                      key={value}
                      type="button"
                      onClick={() => setPublishIdentity(value)}
                      className={cn(
                        "rounded-lg border px-3 py-1.5 text-xs",
                        publishIdentity === value ? "border-primary bg-primary/10 text-primary" : "border-input"
                      )}
                    >
                      {label}
                    </button>
              ...
              <div>
                <label className="block text-xs text-muted-foreground">勋章（可选）</label>
                <div className="mt-1 flex flex-wrap gap-2">
                  {BADGE_OPTIONS.map((b) => (
                    <button
                      key={b}
                      type="button"
                      onClick={() => setPublishBadge(b)}
                      className={cn(
                        "rounded-lg border px-3 py-1.5 text-xs",
                        publishBadge === b ? "border-primary bg-primary/10 text-primary" : "border-input"
                      )}
                    >
                      {b}
                    </button>
              ...
              onSubmit={async (e) => {
                ...
                const max = editingTutorMaxSalary.trim() ? Number(editingTutorMaxSalary) : null;
                  ...
                  university: editingTutorUniversity.trim() || null,
                    ...
                    badge_text: editingTutorBadge || null,
                ...
                <div className="mt-1 flex flex-wrap gap-2">
                  {BADGE_OPTIONS.map((b) => (
                    <button
                      key={b}
                      type="button"
                      onClick={() => setEditingTutorBadge(b)}
                      className={cn(
                        "rounded-lg border px-3 py-1.5 text-xs",
                        editingTutorBadge === b ? "border-primary bg-primary/10 text-primary" : "border-input hover:bg-muted"
                      )}
                    >
                      {b}
                    </button>
